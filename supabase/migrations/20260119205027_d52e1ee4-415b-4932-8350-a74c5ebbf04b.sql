-- Update the notification trigger functions to include project_name and client_name in metadata
...
BEGIN
  ...
  SELECT title, project_name, client_name INTO v_task_title, v_project_name, v_client_name 
      ...
      IF v_admin_ids IS NOT NULL THEN
        FOREACH v_admin_id IN ARRAY v_admin_ids LOOP
          PERFORM public.create_notification(
            ...
            v_project_name || ' • ' || v_client_name || ' - La vidéo "' || v_video_title || '" est prête pour review.',
            '/pm',
    ...
    IF NEW.status = 'review_client' AND OLD.status = 'review_admin' THEN
      ...
      IF v_editor_id IS NOT NULL THEN
        PERFORM public.create_notification(
          ...
          v_project_name || ' • ' || v_client_name || ' - Votre vidéo "' || v_video_title || '" a été validée et envoyée au client.',
          '/editor',
          jsonb_build_object('video_id', NEW.id, 'task_id', NEW.task_id, 'project_name', v_project_name, 'client_name', v_client_name, 'video_title', v_video_title, 'requires_email', true)
        );
      END IF;
    ...
    IF NEW.status = 'revision_requested' THEN
      ...
      IF v_editor_id IS NOT NULL THEN
        PERFORM public.create_notification(
          ...
          'revision_requested',
          'Révision demandée',
          v_project_name || ' • ' || v_client_name || ' - Des modifications ont été demandées sur la vidéo "' || v_video_title || '".',
          '/editor',
          jsonb_build_object('video_id', NEW.id, 'task_id', NEW.task_id, 'project_name', v_project_name, 'client_name', v_client_name, 'video_title', v_video_title, 'requires_email', true)
        );
      END IF;
    ...
    IF NEW.status = 'completed' AND OLD.status != 'completed' THEN
      ...
      IF v_editor_id IS NOT NULL THEN
        PERFORM public.create_notification(
          ...
          'video_completed',
          'Vidéo validée !',
          v_project_name || ' • ' || v_client_name || ' - La vidéo "' || v_video_title || '" a été approuvée par le client.',
          '/editor',
          jsonb_build_object('video_id', NEW.id, 'task_id', NEW.task_id, 'project_name', v_project_name, 'client_name', v_client_name, 'video_title', v_video_title, 'requires_email', true)
        );
      END IF;
      ...
      IF v_admin_ids IS NOT NULL THEN
        FOREACH v_admin_id IN ARRAY v_admin_ids LOOP
          PERFORM public.create_notification(
            ...
            v_project_name || ' • ' || v_client_name || ' - La vidéo "' || v_video_title || '" a été approuvée.',
            '/pm',
            jsonb_build_object('video_id', NEW.id, 'task_id', NEW.task_id, 'project_name', v_project_name, 'client_name', v_client_name, 'video_title', v_video_title, 'requires_email', true)
          );
        END LOOP;
    ...
    IF NEW.status = 'late' AND OLD.status != 'late' THEN
      ...
      IF v_editor_id IS NOT NULL THEN
        PERFORM public.create_notification(
          ...
          'video_late',
          '⚠️ Vidéo en retard',
          v_project_name || ' • ' || v_client_name || ' - La vidéo "' || v_video_title || '" a dépassé sa deadline.',
          '/editor',
          jsonb_build_object('video_id', NEW.id, 'task_id', NEW.task_id, 'project_name', v_project_name, 'client_name', v_client_name, 'video_title', v_video_title, 'requires_email', true)
        );
      END IF;
...
BEGIN
  ...
  -- Get project and client name from task
  SELECT project_name, client_name INTO v_project_name, v_client_name
  ...
  IF v_admin_ids IS NOT NULL THEN
    FOREACH v_admin_id IN ARRAY v_admin_ids LOOP
      PERFORM public.create_notification(
        ...
        'new_delivery',
        'Nouvelle livraison',
        v_project_name || ' • ' || v_client_name || ' - ' || COALESCE(v_editor_name, 'Un éditeur') || ' a livré la version ' || NEW.version_number || ' de "' || v_video_title || '".',
        '/pm',
...
BEGIN
  ...
  IF NEW.assigned_to IS NOT NULL AND (OLD.assigned_to IS DISTINCT FROM NEW.assigned_to) THEN
    ...
    PERFORM public.create_notification(
      ...
      v_project_name || ' • ' || v_client_name || ' - La vidéo "' || v_video_title || '" vous a été assignée.',
      '/editor',
      jsonb_build_object('video_id', NEW.id, 'task_id', NEW.task_id, 'project_name', v_project_name, 'client_name', v_client_name, 'video_title', v_video_title, 'requires_email', true)
    );
  END IF;
